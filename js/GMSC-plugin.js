...
var basetimePrice=GMSC_vars.basetimePrice;
var basetimeDistance=GMSC_vars.basetimedistance;
var extratimePrice=GMSC_vars.extratimePrice;
var extratimeDistance=GMSC_vars.extratimedistance;

var countPriceMode=GMSC_vars.countPriceMode;
	...
	if (navigator.geolocation) {
		navigator.geolocation.getCurrentPosition(function(position) {
            ...
            */
			...
			infoWindow.setContent('<i class="fas fa-map-marker-alt custom-marker-alt"></i><span style="color:#444;font-size:1.5em;font-weight:bold;">'+GMSC_trans.current_position+'</span>');
			infoWindow.open(map);
			map.setCenter(pos);
		}, function() {
		...
		function(response, status) {
			if (status === 'OK') {
				...
				if(countPriceMode=="distance"){
					...
					if(parseInt(duration_amount)<=basetimeDistance){
						price=basetimePrice;
					}
					else{
						price=parseInt(basetimePrice)+parseInt((parseInt(duration_amount)-basetimeDistance)/extratimeDistance*extratimePrice);
					}
				}
...
jQuery(document).on('click','#sendCustomShipOrder',function(){
	...
	if(data["userNm"]==""||data["userPhone"]==""){
		alert(GMSC_trans.name_and_phone_alert);
		return;
	}
	
	if(data["beginpos"]==""||data["endpos"]==""||data["price"]==""){
		alert(GMSC_trans.choose_path_alert);
		return;
	}
	$target=jQuery(this);
	jQuery.ajax({
