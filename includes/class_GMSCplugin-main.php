...
class GMSC_Plugin{
	...
	function AddScripts(){
		...
		wp_localize_script(	
			...
			array(
				...
				'countPriceMode' => get_option('plugin_google_map_counttype_setting'),
				...
				'basetimePrice' => get_option('plugin_google_map_base_time_price'),
				'basetimedistance' => get_option('plugin_google_map_base_time_distance'),
				'extratimePrice' => get_option('plugin_google_map_extra_time_price'),
				'extratimedistance' => get_option('plugin_google_map_extra_time_distance'),
		...
		wp_localize_script(	
			'GMSC-plugin_script', 
			'GMSC_trans',
			GMSC_Translation::get_translation()
			);
	}
	...
	function googleMapOrderExec(){
		...
		if(trim($_POST["exectype"])!=0&&trim($_POST["exectype"])!=1){
			echo json_encode(array('error'=>$translation_array['error_msg']));
			exit();
		}
		...
		if($intReturn==0){
			...
			echo json_encode(array('success'=>$translation_array['success_msg']));
			exit();
		}
	}
	...
	function sendCustomShipOrder(){
		...
		try {
			...
			array_push($obj->Send['Items'], array('Name' => "運費：".$beginpos."->".$endpos, 'Price' => (int)$price,
												  'Currency' => $translation_array['dollar'], 'Quantity' => 1, 'URL' => ""),
					   array('Name' => "手續費", 'Price' => (int)$rent,'Currency' => $translation_array['dollar'], 'Quantity' => 1, 'URL' => "")
					  );
