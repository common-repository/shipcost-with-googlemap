...
class ECPay_Send extends ECPay_Aio
{
    ...
    static function CheckOut($target = "_self",$arParameters = array(),$arExtend = array(),$HashKey='',$HashIV='',$ServiceURL=''){
		...
		//在資料表新增一筆訂單號與檢查碼
		$sql = "update {$wpdb->prefix}ship_custom_order set `szCheckMacValue`='".$szCheckMacValue."' where MerchantTradeNo='".$MerchantTradeNo."'";
...
if(!class_exists('ECPay_CheckMacValue', false))
{
    ...
    class ECPay_CheckMacValue{
        ...
        static function generate($arParameters = array(),$HashKey = '' ,$HashIV = '',$encType = ECPay_EncryptType::ENC_SHA256){
            $sMacValue = '' ;
